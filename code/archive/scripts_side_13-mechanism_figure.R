...
sys_meta <- data.frame(
  ...
  trap_potency = c(NA, NA, 1.0, 100, 0.02, 65, 0.8),  # relative to olaparib
  color        = c("grey40", "darkorange", "#E41A1C", "#FF7F00", "#377EB8", "#4DAF4A", "#984EA3"),
  shape        = c(17, 15, 16, 16, 17, 17, 17),  # triangle for APO/unknown
...
p_2d <- ggplot(combined %>% filter(type != "APO"), aes(x = wd_ratio, y = wd_S1)) +
  ...
  geom_text(aes(label = label), hjust = -0.15, vjust = 0.5, size = 2.5) +
