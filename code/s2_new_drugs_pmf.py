#!/usr/bin/env python3
"""
s2_new_drugs_pmf.py — extension S2 CV1/CV2 well depths (DBE weights)

Purpose:   C1-C3 well depths for the extension-inhibitor S2 systems using DBE
           reweighting (protein-DNA CV1 and HD-ART CV2).
Inputs:    results/analysis/new_drugs_s2/<drug>/{analysis_CV1,analysis_CV2}.dat,
           analysis_weights.dat
...
for d in drugs:
    for cv_name in ["CV1", "CV2"]:
        ...
        w = np.loadtxt(DATA / d / "analysis_weights.dat")
        r = run_pyrew(cv, w)
